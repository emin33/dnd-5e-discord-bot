...
async def test_stale_active_rows_marked_ended(db: Database, repo: SessionRepository):
    """Rows left in any non-terminal state by a crash become 'ended'."""
    for sid, state in [
        ("s-lobby", "lobby"),
...
@pytest.mark.asyncio
async def test_terminal_rows_untouched(db: Database, repo: SessionRepository):
    """'ended' and 'paused' rows are not modified; sweep reports zero."""
    ...
    assert rows == {"s-ended": "ended", "s-paused": "paused"}
